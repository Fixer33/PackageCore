...
namespace CI.WSANative.Store
{
    public static class WSANativeStore
    {
        ...
        private static async void GetAddOnsAsync(Action<WSAStoreProductQueryResult> response)
        {
            ...
            WSAStoreProductQueryResult wsaStoreProductQuery = new WSAStoreProductQueryResult()
            {
                Products = result.Products.ToDictionary(x => x.Key, y => new WSAStoreProduct()
                {
                    ...
                    FormattedPrice = GetFormattedPrice(y.Value),
                    ...
                    SubscriptionInfo = y.Value.Skus.Any(s => s.SubscriptionInfo != null) ? new WSAStoreSubscriptionInfo()
                    {
                        BillingPeriod = (int)y.Value.Skus.First(s => s.SubscriptionInfo != null).SubscriptionInfo.BillingPeriod,
                        BillingPeriodUnit = (WSAStoreDurationUnit)y.Value.Skus.First(s => s.SubscriptionInfo != null).SubscriptionInfo.BillingPeriodUnit,
                        HasTrialPeriod = y.Value.Skus.First(s => s.SubscriptionInfo != null).SubscriptionInfo.HasTrialPeriod,
                        TrialPeriod = (int)y.Value.Skus.First(s => s.SubscriptionInfo != null).SubscriptionInfo.TrialPeriod,
                        TrialPeriodUnit = (WSAStoreDurationUnit)y.Value.Skus.First(s => s.SubscriptionInfo != null).SubscriptionInfo.TrialPeriodUnit
                    } : null
                }),
                Error = result.ExtendedError
            };
        ...
        private static string GetFormattedPrice(StoreProduct product)
        {
            string price = product.Price.FormattedPrice;

            if (WSANativeHelper.IsPriceZero(price))
            {
                foreach (var sku in product.Skus)
                {
                    if (!WSANativeHelper.IsPriceZero(sku.Price.FormattedPrice))
                    {
                        return sku.Price.FormattedPrice;
                    }
                }
            }
